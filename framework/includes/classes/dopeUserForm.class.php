 ...
 */
    ...
    class dopeUserForm extends Form {
        ...
        static public function country_select($countries, $user = null) {
            if( count($countries) >= 1 ) {
                parent::generic_select('countryId', $countries, 'pk_c_code', 's_name', __('Select Country'), (isset($user['fk_c_country_code'])) ? $user['fk_c_country_code'] : null);
            } else {
        ...
        static public function region_select($regions, $user = null) {
            if( count($regions) >= 1 ) {
                parent::generic_select('regionId', $regions, 'pk_i_id', 's_name', __('Select Province or Capital'), (isset($user['fk_i_region_id'])) ? $user['fk_i_region_id'] : null);
            } else {
        ...
        static public function city_select($cities, $user = null) {
            if( count($cities) >= 1 ) {
                parent::generic_select('cityId', $cities, 'pk_i_id', 's_name', __('Select City or Town'), (isset($user['fk_i_city_id'])) ? $user['fk_i_city_id'] : null);
            } else {
            ...
            if(pk_c_code != '') {
                ...
                $.ajax({
                    ...
                    success: function(data){
                        ...
                        if(length > 0) {
                            result += '<option value=""><?php _e("Select Province or Capital"); ?></option>';
                            for(key in data) {
                        ...
                        $("#cityId").html('<option selected value=""><?php _e("Select City or Town"); ?></option>');
                    }
                 });
             } else {
                 // add empty select
                 $("#region").before('<select name="regionId" id="regionId" ><option value=""><?php _e("Select Province or Capital"); ?></option></select>');
                 $("#region").remove();

                 $("#city").before('<select name="cityId" id="cityId" ><option value=""><?php _e("Select City or Town"); ?></option></select>');
                 $("#city").remove();

                 if( $("#regionId").length > 0 ){
                     $("#regionId").html('<option value=""><?php _e("Select Province or Capital"); ?></option>');
                 } else {
                     $("#region").before('<select name="regionId" id="regionId" ><option value=""><?php _e("Select Province or Capital"); ?></option></select>');
                     $("#region").remove();
                 }
                 if( $("#cityId").length > 0 ){
                     $("#cityId").html('<option value=""><?php _e("Select City or Town"); ?></option>');
                 } else {
                     $("#city").before('<select name="cityId" id="cityId" ><option value=""><?php _e("Select City or Town"); ?></option></select>');
                     $("#city").remove();
                 }
            ...
            if(pk_c_code != '') {
                ...
                $.ajax({
                    ...
                    success: function(data){
                        var length = data.length;
                        if(length > 0) {
                            result += '<option selected value=""><?php _e("Select City or Town"); ?></option>';
                            for(key in data) {
                                result += '<option value="' + data[key].pk_i_id + '">' + data[key].s_name + '</option>';
                            }
                            $("#city").before('<select name="cityId" id="cityId" ></select>');
