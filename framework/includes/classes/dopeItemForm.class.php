...
class dopeItemForm extends Form {
        ...
        static public function region_select($regions = null, $item = null) {
            ...
            if( count($regions) >= 1 ) {
                ...
                parent::generic_select('regionId', $regions, 'pk_i_id', 's_name', __('Select state, region or province...'), (isset($item['fk_i_region_id'])) ? $item['fk_i_region_id'] : null);
                return true;
            } else {
                ...
                $.ajax({
                    ...
                    success: function(data){
                        ...
                        if(length > 0) {
                            ...
                            result += '<option value=""><?php echo osc_esc_js(__("Select state, region or province...")); ?></option>';
                            for(key in data) {
                 ...
                 // add empty select
                 $("#region").before('<select name="regionId" id="regionId" ><option value=""><?php echo osc_esc_js(__("Select state, region or province...")); ?></option></select>');
                 $("#region").remove();
                 ...
                 if( $("#regionId").length > 0 ){
                     $("#regionId").html('<option value=""><?php echo osc_esc_js(__("Select state, region or province...")); ?></option>');
                 } else {
                     $("#region").before('<select name="regionId" id="regionId" ><option value=""><?php echo osc_esc_js(__("Select state, region or province...")); ?></option></select>');
                     $("#region").remove();
                 }
